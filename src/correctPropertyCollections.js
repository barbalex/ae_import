  ...
  new Promise((resolve, reject) => {
    ...
    pgDb.any(`
      ...
      .then((data) => {
        ...
        // 1.1 prepare data
        const gisLayerCollections = propertyCollections.filter((pC) => pC.name === `ZH GIS`)
        const gisLayerIds = gisLayerCollections.map((c) => c.id)
        gisLayerPc = {
          name: `ZH GIS`,
          description: `GIS-Layer und Betrachtungsdistanzen für das Artenlistentool, Artengruppen für EvAB, im Kanton Zürich. Eigenschaften aller Arten`,
          links: `{"http://www.naturschutz.zh.ch"}`,
          combining: false,
          organization_id: gisLayerCollections[0].organization_id,
          last_updated: `2016.06.01`,
          terms_of_use: `Open Data: Die veröffentlichten Daten dürfen mit Hinweis auf die Quelle vervielfältigt, verbreitet und weiter zugänglich gemacht, angereichert und bearbeitet sowie kommerziell genutzt werden. Für die Richtigkeit, Genauigkeit, Zuverlässigkeit und Vollständigkeit der bezogenen, ebenso wie der daraus erzeugten Daten und anderer mit Hilfe dieser Daten hergestellten Produkte wird indessen keine Haftung übernommen.`,
      ...
      .then(() => {
        // 2.1 prepare data
        const jahresartenCollections = propertyCollections.filter((pC) => pC.name === `ZH Jahresarten`)
      ...
      .then(() => {
        // 2.1 prepare data
        const schutzCollections = propertyCollections.filter((pC) => pC.name === `FNS Schutz (2009)`)
